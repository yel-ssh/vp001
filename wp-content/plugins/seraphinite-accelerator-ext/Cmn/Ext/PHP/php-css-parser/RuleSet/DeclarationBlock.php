...
class DeclarationBlock extends RuleSet {
	...
	public function parseSelectors(ParserState $oParserState) {
...

		{
			...
			for( ;; )
			{
				...
				if( $sSym == ')' )
				{
					...
					if( $nBracket < 0 )
					{
						$oParserState->traceException(new SrcExcptn(Settings::ParseErrMed, "Unopened bracket in selector", $oParserState->currentPos()));
						$nBracket = 0;
					}
				...
				if( $nBracket > 0 )
					$oParserState->traceException(new SrcExcptn(Settings::ParseErrMed, 'Unclosed ' . $nBracket . ' bracket(s) in selector', $iStartPos));
		...
		if(!$oParserState->comes('{')) {
			if($oParserState->comes(';'))
				$oParserState->consume(1, false);
			throw new SrcExcptn(Settings::ParseErrMed, "Unexpected end of selector", $oParserState->currentPos());
		}
		$oParserState->consume(1, false);
